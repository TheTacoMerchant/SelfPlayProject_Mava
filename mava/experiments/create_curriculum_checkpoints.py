...
def run_experiment(_config: DictConfig) -> float:
    ...
    ckpt_steps = list(range(config.arch.num_evaluation//5, config.arch.num_evaluation, config.arch.num_evaluation//5)) + [config.arch.num_evaluation]
    ...
    for eval_step in range(config.arch.num_evaluation):
        ...
        if eval_step in ckpt_steps:
            ...
            # )
            ckpt = {"learner_state": unreplicate_n_dims(learner_output.learner_state)}
            save_args = orbax_utils.save_args_from_target(ckpt)
            ckpt_dir = pathlib.Path(f'../checkpoints/curriculum/ckpt_{ckpt_steps.index(eval_step)}').absolute()
