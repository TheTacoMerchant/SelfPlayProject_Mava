...
from mava.experiments.smax.distributions import (
...
MAP_NAME_TO_SCENARIO = {
    ...
    "smacv2_30_units_custom": Scenario(jnp.zeros((60,), dtype=jnp.uint8), 30, 30, False, True),
}
...
class SMAX(MultiAgentEnv):
    ...
    def reset(self, key: chex.PRNGKey, *args) -> Tuple[Dict[str, chex.Array], State]:
        ...
        team_1_start = jnp.stack([jnp.array([self.map_width / 4 + self.map_width / 2 * (1-swap), self.map_height / 2])] * self.num_enemies)
    ...
    def get_obs_unit_list(self, state: State) -> Dict[str, chex.Array]:
        ...
        def get_features(i, j):
            ...
            j_idx = jax.lax.cond(
                i < self.num_allies,
                lambda: j,
                lambda: (self.num_allies + j) % (self.num_agents-1),
            )
            j_idx = jax.lax.cond(
                j_idx < i,
                lambda: j_idx,
    ...
    def get_avail_actions(self, state: State) -> Dict[str, chex.Array]:
        ...
        def get_individual_avail_actions(i, team):
            ...
            shootable_mask = (
                ...
                if team == 0
                else shootable_mask[: self.num_allies]
            )
