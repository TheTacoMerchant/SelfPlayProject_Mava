...
def get_league_eval_fn(env, actor_apply_fn, config):
    eval_episodes = config.arch.num_eval_episodes

    def act_fn(
        params: FrozenDict, obs, key: PRNGKey
    ) -> Tuple[Action, Dict]:
        pi = actor_apply_fn(params, obs)
        action = pi.mode() if config.arch.evaluation_greedy else pi.sample(seed=key)
        return action

    def eval_fn(ally_params: FrozenDict, league_params, key: PRNGKey) -> Metrics:
        ...
        def eval_pair(enemy_params):
            def _env_step(eval_state: _EvalEnvStepState, _) -> Tuple[_EvalEnvStepState, TimeStep]:
                """Performs a single environment step"""
                env_state, ts, key = eval_state

                key, act_key = jax.random.split(key)
                obs = ts.observation
                ally_obs = jax.tree.map(lambda x: x[:,:env.num_allies], obs)
                enemy_obs = jax.tree.map(lambda x: x[:,env.num_allies:], obs)
                ally_action = act_fn(ally_params, ally_obs, act_key)
                enemy_action = act_fn(enemy_params, enemy_obs, act_key)
                actions = jnp.concat([ally_action, enemy_action], axis=-1)
            ...
            def _episode(key: PRNGKey) -> Tuple[PRNGKey, Metrics]:
                ...
                _, timesteps = jax.lax.scan(_env_step, step_state, length = env.time_limit+1)
                ...
                metrics = tree.map(lambda m: m[done_idx, jnp.arange(eval_episodes)], metrics)
            ...
            # Note, we're not looping rn
            _, metrics = _episode(key)
            # metrics = tree.map(lambda x: x.reshape(-1), metrics)  # flatten metrics
            metrics["win_rate"] = 100 * jnp.mean(metrics["won_episode"])
            return metrics
        
        eval_vmap = jax.vmap(eval_pair)
        metrics = eval_vmap(league_params)
        return {"win_rate" :metrics["win_rate"]}
        

    def timed_eval_fn(ally_params: FrozenDict, league_params, key: PRNGKey) -> Metrics:
        """Wrapper around eval function to time it and add in steps per second metric."""
        n_league_members = league_params['params']['action_head']["Dense_0"]['bias'].shape[0]
        start_time = time.time()

        metrics = eval_fn(ally_params, league_params, key)
        metrics = jax.block_until_ready(metrics)

        end_time = time.time()
        total_timesteps = (env.time_limit+1) * n_league_members * eval_episodes
...
def get_eval_fn(
    ...
    def eval_fn(params: FrozenDict, key: PRNGKey, init_act_state: ActorState, env_state) -> Metrics:
        """Evaluates the given params on an environment and returns relevent metrics.
